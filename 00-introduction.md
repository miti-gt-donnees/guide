...
- les 2 premières phases "{ref}`imaginer` et {ref}`concevoir-planifier`", sont les étapes préparatoires d'un projet, où l'on se préoccupe d'avoir toutes les informations nécessaires à la bonne gestion des données et du projet. C'est l'étape où l'on réfléchi au de plan de gestion de données, où l'on prépare les organes de stockage, et où l'on met en place les outils de gestion de projet. 
[Cette partie doit aider le lecteur qui veut améliorer sa gestion des données, à se poser les bonnes questions et à savoir puiser les infos/réponses dans des ressources. A REMPLACER PAR LA PROPOSITION + BAS]
Cette partie, très générique et englobante a pour objectif de conduire le lecteur à s'interroger sur ses besoins, les moyens dont il dispose, à se poser les bonnes questions et à s'orienter pour trouver des solutions adaptées dans un enivronnement riche, mouvant à surveiller.
