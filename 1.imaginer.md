...
La présentation de Chloé Martin (ANF Rbdd 2016) nous renseigne plus particulièrement sur **droit applicable au bases de données** (droit suis généris) mais aborde également la notion de **droit d’auteur** et celui des **agents publics**. 


*(Voir si on garde ou pas la présentation de Chloé Martin ?)*
...
En complément de ces présentations, il est à retenir que le droit des producteurs de bases de données (droit sui generis) est désormais neutralisé par la Loi sur la république numérique. Comme toute administration, les universités et Etablissements de recherche ne peuvent opposer leur droit de producteur de base de données à la libre réutilisation des informations qu'elles produisent. Le principe d'ouverture par défaut s'applique.
