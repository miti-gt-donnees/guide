...
Dans cette phase du cycle de vie des données les réseaux se préoccupent du traitement des données brutes issues des acquisitions.
Il s'agit souvent de regrouper, choisir, qualifier les données pertinentes, de les reformater dans des formats standards interopérables, et d'associer les métadonnées nécessaires :

Les réseaux métiers participent au traitement des jeux de données via la mise en place de procédures ou de logiciels assurant la qualité des données et la qualité des traitements. Cela comprend par exemple :
- la modélisation et le reformatage des données dans des formats standards ineropérables; 
- la structuration en base de données ; 
- la mise à disposition de moyens informatiques, plateformes d'hébergement de données; 
- l'aide aux choix des solutions de traitement les plus adaptées, 
- la détection des erreurs de saisie ou des incohérences relatives aux modèles de données, 
- la mise en place d’un contrôle qualité

## Préparation des données : Structurer et organiser les données
...
[CH -*Dans collecter certains outils présentés permettent aussi en plus de la collecte des donnée de traiter les jeux de donner - Penser si on garde le paragraphe à faire un renvoi ???*]
...
Dans le domaine environnemental et particulierement pour les données atmosphériques et océanographiques, le format de fichier [NetCDF](https://www.unidata.ucar.edu/software/netcdf/)  est particulièrement interessant pour représenter des données qui issues de séries temporelles, de profils verticaux ou de trajectoires. Ce format est très utilisé en sciences de l'environnement par exemple en océanographie, météorologie, atmosphere. 

Ce format de fichier présente l'avantage de fournir des données autodocumentées, grace une entete qui permet d'inscrire un grand nombre de métadonnées dans le fichier lui même.

Ainsi les métadonnées sont insérées et véhiculées dans le fichier avec les données elles memes.
On peut ainsi décrire de manière assez précise les données du fichier en inscrivant les métadonnées telles que les unités de mesure, la licence, les proprietaires etc, ainsi que l'organisation des données, sans avoir besoin d'un fichier de description complémentaire. 


Toutefois là encore si les métadonnées sont bien utiles dans la description d'un dichier NetCDF, le format n'impose rien et on peut y mettre ce qu'on veut, et donc une standardisation des métadonnées NetCDF est utile et nécessaire. Il est recommendé d'utiliser la [convention CF (climate forecast)](https://cfconventions.org/) qui permet de standardiser autant que faire se peut les métadonnées
...
### Traitement des données par langage informatique
