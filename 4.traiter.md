...
*  Action  nationale de formation du réseau SIST http://sist.cnrs.fr
...
*  le serveur cartographique *Geoserver* permet d'afficher et permettre les échanges de données géospatiales sur le web selon les standards (WMS, WFS, ...) de l'OGC ;
    ...
    l'application GeoCMS permettant la visualisation de données géospatiales sur le web et de mettre en place une Infrastructure de Données Géographique (IDG) 
     ...
     des logiciels comme Thredds et Erddap sont des plateforme web de gestion qui permettent une gestion FAIR pour la diffusion des données 
...
Ce format présente l'avantage d'etre autodocumenté grace une entete qui permet d'inscrire un grand nombre de métadonnées.
...
Le format ODV (ocean data view) est également un format intéressant. Il se rapproche d'un format CSV i.edes colonnes de données séparées par des virgules (ou tout autre séparateur)
