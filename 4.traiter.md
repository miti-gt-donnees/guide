...
Dans les domaines Océan, Atmosphère par exemple, le format [NetCDF](https://www.unidata.ucar.edu/software/netcdf/) est un format ouvert, autodocumenté et très utilisé en particulier dans les communautés sciences de l'environnement par exemple pour formater des données qui sont issues de mesures sur le terrain et représentent des profils verticaux, des séries temporelles, ou encore des trajectoires...), sans avoir besoin d'un fichier de description complémentaire. 

Le format NetCDF est autodocumenté par il permet d'insérer les métadonnées dans un entête du fichier, avec les données elles-mêmes.
...
Toutefois dans son format originel NetCDF n'impose rien comme métadonnées, et on peut y mettre ce qu'on veut comme libellé de variables, unités, etc., ce qui nécessite donc une standardisation des métadonnées NetCDF. À cet effet la  [convention CF (climate forecast)](https://cfconventions.org/) propose et fournit une standardisation des variables et unités de mesures à inscrire dans un fichier NetCDF.
