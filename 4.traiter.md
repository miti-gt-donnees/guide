...
## Contexte 
Cette étape du cycle de vie des données est destinée à traiter les données brutes, regrouper, choisir les données pertinentes, reformater, gérer les métadonnées :
...
Parmi les premiers traitements opérés sur des données brutes, on doit fréquemment filtrer, extraire les données utiles parmi un ensemble
Les données issues de capteurs environnementaux sont fréquemment dans des formats propriétaires, souvent illisibles par un être humain.Il convient alors de traiter les fichiers bruts
de manière à en extraire les données utiles et les réécrire dans des formats utilisables par une communauté.

Un exemple de traitement de fichiers provenant de capteurs du projet EMSO 
...
est particulièrement interessant pour représenter des données qui fot partie de séries temporelles, de profils verticaux ou de trajectoires.
Il est très utilisé en sciences de l'environnement par exemple en océanographie, météorologie, atmosphere.

Ce format présente l'avantage d'etre autodocumenté grace une entete qui permet d'inscrire un grand nombre de métadonnées dans le fichier lui même.
  ...
  * <https://sist19.sciencesconf.org/data/pages/SIST19_Atelier_NetCDF_python.pdf>
   ...
   * Réseau d'observation du Pacifique Sud ‘ReefTEMPS' : évolutions fonctionnelles et optimisation d'un système d'information dédié capteurs et reconstitution de séries temporelles                   
       *   <https://sist16.sciencesconf.org/data/pages/12\_R\_Hocde.pdf>                     
           ...
           - 
Le format ODV (ocean data view) [https://odv.awi.de/](https://odv.awi.de/)  est également un format intéressant. Il se rapproche d'un format CSV i.e des colonnes de données séparées 
...
Des logiciels sont plus particulièrement adaptés dans la diffusion et d'affichage des données scientifiques d'observation, de plus en utilisant les standards interopérables de l'Open Geospatial Consortium (OGC), ISO19139, Inspire, etc)
...
*  L'utilisation de ces logiciels fait l'objet d'une  Action  nationale de formation du [réseau SIST](http://sist.cnrs.fr) qui permet de savoir installer, configurer et utilsier ces logiciels
