...
Bien souvent, les données primaires sont issus d'instruments, de collecte sur le terrain, ... et se présentent sous la forme de fichiers aux formats propriétaires peu interopérables.

Il est important de se préoccuper du format des données afin qu'ils soient ouverts et interopérables. A à cet effet, le site Doranum propose une [introduction à la définition de formats ouverts ou fermés](https://doranum.fr/stockage-archivage/quiz-format-ouvert-ou-ferme/).
...
Dans les domaines Océan, Atmosphère par exemple, le format [NetCDF](https://www.unidata.ucar.edu/software/netcdf/) est un format ouvert, auto-documenté et très utilisé en particulier dans les communautés sciences de l'environnement par exemple pour formater des données qui sont issues de mesures sur le terrains et représentent des profils verticaux, des séries temporelles, ou encore des trajectoire...), sans avoir besoin d'un fichier de description complémentaire. 

Le format NetCDF est autodocumenté par il permet d'insérer les métadonnées dans une entete du fichier, avec les données elles mêmes.
On peut ainsi décrire de manière assez précise les données du fichier, par exemple en insérant les unités de mesure des parametres mesurés, la licence de diffusion, les propriétaires etc, ainsi que l'organisation des données.

Toutefois dans son format originel NetCDF n'impose rien comme metadonnées, et on peut y mettre ce qu'on veut comme libellé de variables, unités, etc., ce qui nécessite donc une standardisation des métadonnées NetCDF. A cet effet la  [convention CF (climate forecast)](https://cfconventions.org/) propose et fournit une standardisation des variables et unités de mesures à inscrire dans un fichier NetCDF.


Les formats NetCDF et ODV sont les formats recommandés et utilisés par le pôle de données Odatis  <https://www.odatis-ocean.fr/donnees-et-services/principes-de-gestion-des-donnees/formats-attributs-conventions> et par le projet Européen Seadatanet <https://www.seadatanet.org/>, et  a été présenté au [séminaire SIST19 à l'OMP](https://sist19.sciencesconf.org) de Toulouse, par Joel Sudre, Maurice Libes et Didier Mallarino :
...
Ce logiciel "Talend" a été également utilisé par Soumaya Lahbib pour traiter les fichiers de données issus des capteurs du [projet EMSO Ligure-ouest](https://www.osupytheas.fr/?Presentation-du-projet-EMSO) et les transformer en fichier CSV utilisables facilement.
...
On peut aussi déposer et faire gérer des données via des serveurs cartographiques  comme geoserver, 
...
decoder s on laisse ou enleve ce passage]
