...
Parmi les premiers traitements opérés sur des données brutes provenant du terrain, on doit fréquemment filtrer, extraire les données utiles dans des fichiers issus des acquisitions par des capteurs.
...
est particulièrement interessant pour représenter des données qui issues de séries temporelles, de profils verticaux ou de trajectoires.
Ce format est très utilisé en sciences de l'environnement par exemple en océanographie, météorologie, atmosphere. 
...
Ce format est bien adapté pour décrire des mesures acsquise selon le temps (série temporelle), selon un axe vertical (profil), selon un déplacement horizontal (trajectoire), ou sur des points fixes.


Toutefois là encore si les métadonnées sont bien utiles dans la description d'un dichier NetCDF, le format n'impose rien et on peut y mettre ce qu'on veut, et donc une standardisation
...
<https://sist15.sciencesconf.org/program> (mettre le lien exact vers le fichier)
Mark Hoebeke, Station biologique de Roscoff                      
