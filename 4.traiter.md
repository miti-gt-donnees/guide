...
### Rendre les fichiers de données interopérables
   ...
   * mise en forme des données dans des formats standards, interopérables NetCDF, ODV, HDF, etc..
...
Les données issues de capteurs environnementaux sont fréquemment dans des formats propriétaires, souvent illisibles et peu exploitables par un être humain.Il convient alors de traiter les fichiers bruts
de manière à en extraire les données utiles et les réécrire dans des formats standards utilisables par une communauté.


Le format de fichier [NetCDF](https://www.unidata.ucar.edu/software/netcdf/)
est particulièrement interessant pour représenter des données qui font partie de séries temporelles, de profils verticaux ou de trajectoires.
...
Un exemple de traitement de fichiers provenant de capteurs du projet EMSO :  les fichiers sont traités par le logiciel ETL Talend et convertis en format CSV
...
*  Les logiciels comme Thredds et Erddap sont des plateformes web de gestion de données particulièrement adaptée pour rendre les données FAIR dans un établissement et faciliter 
