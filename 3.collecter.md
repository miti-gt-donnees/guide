...
Deux solutions, étudiées au CEFE, ont été présentées à l'occasion de cette ANF : le développement d'une application nomade basée sur QGIS ainsi qu'une application nomade utilisant ODK. C'est d'ailleurs la solution basée sur ODK qui a été utilisée lors de l'ANF « Interfacer les outils mobiles avec son système d’information » cité précédemment car la solution ODK permet de couvrir les étapes allant de la création du formulaire à la sécurisation en bases de données.
