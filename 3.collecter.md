...
Cette phase du cycle de vie de la donnée, concerne l'acquisition et collecte des données, le recueil et la constitution des jeux de données ("dataset" en anglais) avec leur métadonnées descriptives. 
Il s'agit donc dans cette phase de travailler aux processus  d'acquisition, ou à la création des données au moyen de divers media (capteurs environnementaux, instruments, sondages, 
modèles numériques...)  selon le domaine étudié, pour constituer des jeux de données. 

Une fois les données acquises il est nécessaire et indispensable dans une optique FAIR de les décrire avec leurs métadonnées associées. 
En effet des données non décrites par des "métadonnées" sont inutilisables.

La description  de ces jeux de données nécessite d'utiliser autant que faire se peut  des référentiels (thésaurus) si possible standardisés, les plus appropriés selon le domaine étudié.

Il est conseillé de gérer les jeux de données dans un environnement technique qui permette d’ assurer le stockage, la sauvegarde, le «versionning»,  l’accessibilité et
l'interopérabilité des données. Cette gestion se fait via des infrastructures techniques, des bases ou des supports qui doivent etre fiables et bien documentés, 
et dans le respect des règles de traitement spécifiques des données personnelles.
...
Dans une optique de gestion FAIR des données, il est nécessaire autant que faire se peut, et lorsqu'elles existent, de suivre des normes et des standards dans divers secteurs comme
...
* [Formats et métadonnées](https://anfdonnees2016.sciencesconf.org/data/pages/2016_07_07_ANF_Renatis_Formats_Standards_et_Metadonnees_1.pdf) ([voir aussi la vidéo](https://youtu.be/obGDFrXyBiU?t=3))  
...
Par exemple, dans le domaine environnemental et pour des données qui sont souvent géolocalisées (nécessité de savoir d'où proviennent les données), l'OGC "Open Geospatial consortium" est l'organisme international qui à l'instar du WWC pour le Web,  edicte et propose des
...
GeoNetwork [*CH - Préciser ce qu'est GeoNetwork*] prend en charge les normes ISO19115, ISO 19139 utilisées pour les ressources spatiales ainsi que le format standard *"Dublin Core"* pour décrire les jeux de données et en faire un portail de données ouvertes.
...
Le [réseau SIST](http://sist.cnrs.fr) a  organisé 2 actions de formation nationale (ANF) sur ces logiciels mettant e noeuvre les standards d'interopérabilité WMS, CSW, SOS : <https://sist.cnrs.fr/les-formations> 
...
### Normes de métadonnées et d'information Géographique [*CH - Je ne sais trop quoi penser de ce titre ?*]
...
### Activités de recherche et gestion des connaissances [*CH : Je changerais ce titre car c'est le titre de la présentation *]
...
[ML: un peu court ici, sais pas ce qu'on peut dire de bien intéressant?]

[CH - * Le titre "Maîtriser l'acquisition de la collecte des données" appelle différentes approche et à défaut de pouvoir rester sur les aspects techniques de la collecte on pourrait éventuellement introduire la notion de métadonnée développée plus haut pour expliquer qu'une bonne collecte nécessite de recourir à des standards et des normes adaptées aux discipline et donner l'exemple des standards de l'OGC en science de l'environnement ?*]
...
### Web scrapping, collecte automatique et analyse de données [*Partie Ajoutée par CH - Revoir le titre en fonction du reste ? En attente de relecture par un collègue que j'ai sollicité*]
...
### Respecter le RGPD [CH *Voir aussi la partie Imaginer*]

*Le Règlement Général sur la Protection des Données* (RGPD) est une modernisation fondamentale des lois européennes sur la protection des données qui place l’idée de confidentialité
...
### Utiliser des Cahiers de laboratoire [CH-*Pourrait également être une sous partie de maitriser l'acquisition*] 

[CH-*Serait-il pertinent de regrouper cahier de labo et carnet de terrain sous un titre du type "Organiser la collecte pour une meilleure réutilisation des données" ?]
...
[ML: ici ou dans imaginer: dans le paysage natioanl de la gestion des donénes??]

[CH -*Cette partie la peut peut-être remonter dans "Maitriser l'acquisition et la collecte" aux côtés du web scrapping ?*]
...
#### les standards de l'OGC en sciences de l'environnement [CH- *Attention - deux fois le même titre*]
