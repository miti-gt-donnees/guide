...
[MCQ - début : dans la présentation de Marie Puren citée précédemment, il y a des pages sur les standards de métadonnées (page 49 à 55) et page 56 elle explique comment les trouver (les liens n'ont pas l'air de marcher). Pour ma part je fais souvent référence à celui de dcc https://www.dcc.ac.uk/guidance/standards/metadata). Dans ce guide, je ne pense pas que ca soit utile de détailler quelques standarts comme c'était prévu mais peut être plus tot d'expliquer comment les trouver et d'insister sur le fait que même si c'est un standart il n'est pas à utiliser que si il est reconnu dans sa discipline. De plus il faut peut être indiquer que le choix d'un standart est certes important mais qu'il existe des "moulinettes" pour passer d'un standart à l'autre. : fin - MCQ]
- **Le Darwin Core** : [MCQ ??]
...
* [La Directive INSPIRE pour les néophytes](http://formations-geomatiques.developpement-durable.gouv.fr/MIG/directive_inspire_neophytes/res/Directive_inspire_neophytes_papier.pdf)  
    F. Merrien, M. Léobet, M. Francès
     Mission de l'information géographique du ministère de l'environnement 
...
[MCQ - début : je ne comprend pas pourquoi le standart iso 1995 n'est pas à la suite de Dublin Core. C'est un autre exemple de standart -: fin - MCQ ]
...
La norme ISO 19139 est l'implémentation XML de la norme ISO 19115. Elle définit le codage XML des méta-données géographiques, une implémentation de schéma XML dérivée de la norme ISO 19115. La norme ISO 19139 est le modèle principal utilisé pour décrire des données dans le logiciel GeoNetwork et constituer ainsi un catalogue de données géospatialisées.

**Les données dans le domaine environnemental**
[MCQ - début : pour moi tout ce passage concerne le catalogage et pas les métadonnées et perso je trouve cela trop long - je freai bien une annexe 97 - les données en information géographique : fin - MCQ]
...
C'est d'ailleurs la solution basée sur ODK qui a utilisé lors de l'ANF « Interfacer les outils mobiles avec son système d’information » cité précédemment car la solution ODK permet de couvrir les étapes : de la création du formulaire à la sécurisation en bases de données. Deux interventions concerne plus particulièrement la collecte de donnée : 
