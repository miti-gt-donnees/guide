...
Il s'agit de travailler à l'acquisition des données au moyen de divers media (capteurs environnementaux, instruments, sondages...) selon le domaine étudié, 
pour constituer des jeux de données, et les décrire avec leurs métadonnées associées. 

La description standardisée de ces jeux de données nécessite d'utilsier des référentiels (thésaurus) les plus appropriés selon le domaine étudié.

Les jeux de données  sont alors gérés dans un environnement technique leur permettant d’assurer le stockage, le «versionning», et l’accessibilité eet l'interopérabilité des données 
et des jeux de données  via des bases ou des supports fiables et bien documentés, dans le respect des règles de traitement spécifiques des données personnelles.

Cette phase nécessite d'étudier :

   * la nécessité de metadonnées précises pour la description des capteurs de mesures et des dispositifs d'acquisition et des données elles mêmes
...
Dans le domaine environnemental et des données géolocalisées, l'OGC "Open Geospatial consortium" est l'organisme international qui à l'instar du WWC pour le Web,  edicte et propose des
...
-  **[WMS](https://www.ogc.org/standards/wms)** - Web Map Service est un protocole de communication standard qui permet d'obtenir des cartes de données géoréférencées 
   ...
   <https://fr.wikipedia.org/wiki/Web_Map_Service>. Ce protocole est notamment utilisé dans le logiciels GeoNetWork et GeoServer

-    **CS-W** - Catalog Service Web : Interrogation de catalogues de métadonnées. Le protcole est destiné à diffuser et explorer les fiches de métadonnées présentes dans un
catalogue. Une très bonne implémentation de ce protocole est réalisée dans le logiciel geonetwork <https://geonetwork-opensource.org/> utilsié pour constituer des
catalogues et inventaires de jeux de données.

GeoNetwork  prend en charge les normes ISO19115, ISO 19139 utilisées pour les ressources spatiales ainsi que le format Dublin Core pour décrire les jeux de données
...
-    **SOS** - Sensor Obervation Service: La norme SOS est destinée à diffuser et distribuer les données et de métadonnées de capteurs de manière interopérable.
   ...
   Ce standard définit une interface de service Web qui permet d'interroger les observations, les métadonnées des capteurs, ainsi que les représentations des caractéristiques observées. 
    ...
    Elle définit également les opérations permettant d'insérer de nouvelles observations de capteurs. Cette norme définit cette fonctionnalité de manière indépendante des liaisons 
    ; deux liaisons sont spécifiées dans ce document : une liaison KVP et une liaison SOAP.
...
- istSOS : istSOS est une implémentation de serveur OGC SOS écrite en Python. istSOS permet de gérer et d'envoyer des observations provenant de capteurs de surveillance selon 
...
Le réseau SISTa  organisé 2 actions de formation nationale (ANF) sur ces logiciels : <https://sist.cnrs.fr/les-formations> 
...
D'autres standards OGC sont également proposés et en 
