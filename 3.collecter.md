...
Il est à noter que la collecte sur le terrain nécessite de s'équiper d'un matériel apte à être utiliser sur des terrains parfois hostiles. Le choix de l'équipement conditionne aussi le choix de la solution logiciel comme le montre Marie-Claude Quidoz lors de cette présentation.
:::{admonition}Réference
...
La duplication des données par stockage redondant sur des supports différents de ceux de l'équipement utilisé (poste de travail fixe, mobile, serveur, ...) est un des principes de base d’une bonne conservation. Il convient de préférez un archivage centralisé conformément à la règle du 3-2-1 généralement recommandée (3 copies sur 2 supports différents, dont 1 lieu déporté). 
