...
Le logiciel GeoNetwork utilise le protocole WMS (Web Map Service) de l'OGC pour pouvoir interagir avec d'autres logiciels cartographiques comme [GeoServer](http://geoserver.org/) afin de représenter les points de mesures sur une carte et de pouvoir accéder aux données brutes associées à un point de mesure Géolocalisé.
...
non descripteurs) représentant les concepts d'un domaine de la connaissance.

De nombreux thesaurus existent, cependant selon le domaine scientifique certains doivent être créés ou sont en perpétuelle évolution

Baptiste Laporte s'est penché sur la création d'un thésaurus collaboratif
