...
Cette dernière étape d’un projet  de  recherche représente un peu la finalité de toute une politique de gestion de données puisqu'elle vise à diffuser, publier, réutiliser des données de qualité selon des formats et des processus interopérables.
...
Ainsi les réseaux travaillent sur l’ensemble des informations (données, modes opératoires, échantillons, publications, visualisation et interfaces graphiques) nécessaires à la mise en œuvre des supports de diffusion et de valorisation les plus pertinents en rapport avec l’objectif du projet initial

Cette étape de publication et de diffusion est souvent accompagnée d'une action d'*identification* des données via des identifiants pérennes, et de dépots dans des entrepots de données.
...
En fin de projet il est nécessaire de mettre à jour et finaliser la rédaction du plan de gestion de données. Vérifier que lespremières informations qui ont été rédigée sont encore valides et valables, et mettre à jour en rajoutant les dernières informations nécessaires
...
Le logiciel GeoNetwork utilise le protocole WMS (Web Map Service) de l'OGC pour interagir avec le logiciel [GeoServer](http://geoserver.org/) afin de représenter les points de mesures sur une carte et de pouvoir accéder aux données brutes
associées a un point de mesure Géolocalisé.

L'utilisation du logiciel GeoServer a été traitée précisément par une action de formation du [réseau SIST](https://sist.cnrs.fr/). Pour lequel on trouvera les informations
* - [https://sist.cnrs.fr/les-formations/anf-2017-1](https://sist.cnrs.fr/les-formations/anf-2017-1)
...
Afin d'être citées et réutilisées, les données doivent disposer d'un *identifiant perenne* pour qu'elles soient référencées de manière univoque, visibles et accessibles au même titre que les publications.  

Il existe différents types d'identifiants pérennes pour toutes sortes d'objets y compris les humains. Cet article fait le point sur les identifiants des documents numériques.
...
Les D.O.I (Digital Object Identification) permettent que les données soient plus facilement citables et  que
l'on puisse les lier aux publications ou à tout autre produit de recherche. Ils concourent donc à l'identification et à l'interopérabilité des données.  

L'obtention de D.O.I implique des devoirs de la part du déposant, qui est de maintenir un lien permanent vers les données identifiées pendant une certaine durée, à travers une page de description (appelée aussi "*landing page*") qui permet de fournir les métadonnées principales pour décrire les données et y accéder.

Les fiches d'un catalogue GeoNetwork peuvent par exemple servir de "landing page" pour des données identifiées par un D.O.I
Exemple:
- https://doi.org/10.34930/5c9f6377-726b-436b-aa0f-ecc32803ef88

Le portail Seanoe utilisé dans le pôle de données Odatis fournit également un service d'identification et de dépot de données marine
