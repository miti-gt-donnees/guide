...
Cette étape finale d’un projet  de  recherche est un peu la consécration de toute une politique de gestion de données puisqu'elle vise à diffuser, publier, partager
des données de qualité selon des processus interopérable et donc réutilisables.

L’accompagnement  des  réseaux s’exerce sur la documentation des données et leurs formats d’exploitation, pour en permettre l’accès et en assurer la reproductibilité ou la
réplicabilité. 
...
Le data paper est un article scientifique : il permet de décrire un jeu de données de recherche (data, dataset), à l’aide d’informations plus précises que celles qu'on peut trouver dans un DMP. 

    il insiste sur la qualité des données et de leur méthode de collecte et de traitement.
    ...
    il donne généralement accès au jeu de données, dans un fichier attaché ou par un lien pérenne (URL, DOI) vers l’entrepôt (data repository, ou repository of research data) où le jeu est déposé.
