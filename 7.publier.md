...
Contexte: Etape finale d’un  projet  de  recherche  impliquant  une
...
Dans cette présentation en quatre parties, Esther Dzalé Yeumo, définit dans un premier temps **les entrepôts de données** et présente quelques **éléments de contexte** 
(omniprésence des données,  contexte réglementaire et scientifique où il est indiqué que l’accessibilité aux données scientifiques devient un enjeu important pour 
la recherche). Il existe différents canaux de mise à disposition des données et surtout différents types d’entrepôt de données (disciplinaires, thématiques, 
pluridisciplinaires). Elle explique ce qu’est la **curation des données**, en quoi consiste les **fonctionnalités et services des entrepôts de données** et introduit la 
notion de **cycle de vie de la donnée** en précisant que l’archivage des données est un processus et pas un simple dépôt à la fin d’un projet. Elle aborde ensuite la 
**notion d’entrepôt de confiance et de certification**. La deuxième partie de son exposé est dédié au **choix d’un entrepôt** : **comment rechercher un entrepô****t**, quels critères
de **choix** prendre en considération. Elle explique dans sa troisième partie **comment déposer dans un entrepôt de données **: comment **préparer ses données** (sélectionner, 
structurer, organiser ses données, vérifier les formats, les volumes de fichier, vérifier le cadre légal), comment documenter ses données (en privilégiant les 
standards et terminologies du domaine). Elle propose des guides méthodologiques sur les **formats ouverts, et le choix d’une licence**. La quatrième et dernière partie 
est consacrée  à la **valorisation du dépôt** (comment faire connaitre ses données, les politiques éditoriales des éditeurs).
...
Dans cette présentation, Sylvie Cocaud  explique les enjeux liés à la gestion et au partage des données de la recherche en précisant qu’ils nécessitent des outils 
appropriés. Qu'est-ce qu'un **entrepôt de données** et quelles en sont les **principales caractéristiques** ? Elle définit dans un premier temps les **entrepôts et données de 
la recherche**, les **métadonnées et jeux de données** puis elle aborde les **fonctionnalités des entrepôts** à savoir le dépôt et la conservation des données, l’identification
pérenne des données, la description des données, le contrôle des droits d’accès et les conditions d’utilisation, la recherche, l’affichage et l’export de (méta)données, 
l’exploration et la visualisation des données, la découverte et la visibilité des données. Elle donne ensuite quelques e**xemples d’entrepôts et de dépôts**. Elle poursuit 
sa présentation sur le **choix des entrepôts** (critères et recommandations), les **principes FAIR**, les **répertoires disponibles et la notion de certification**. Elle aborde 
ensuite la question de la p**lace de l’entrepôt dans le cycle du projet de recherche**.


## Déposer/Publier dans des Plateformes logicielles de gestion de données locales - (Infrastructure de données géographiques)

Les services informatiques des laboratoires de recherche et des OSU
ont pour vocation et pour mission d'intervenir dans la gestion des
données d'observation acquises sur le terrain.  Après les phases
de collecte et de transfert de données que nous avons vues dansles
précédentes étapes du cylce de vie des données, il est nécessaire
"in fine" de se préoccuper de la valorisation des données en
permettant la facilité d'accès et leur réutilisation.

Un certains nombre de logiciel font office de plateforme d'accès
et de gestion des données. Ils permettent de présenter les données
avec leur métadonnées, de fournir des interfaces de recherche, de
géolocaliser les données, et parfois de visualisation des données
avec des graphes

La gestion des données demande donc aux informaticiens concernés
de développer des compétences dans des domaines très divers (gestion
de la production des données, organisation de la donnée, gestion
de la géolocalisation, gestion de la donnée dans le temps, mise à
disposition et visualisation des données, modèle OAIS, ...).

Les événements suivants organisés par le réseau SIST fournissent
un certain nombre de connaissances sur l’utilisation d'infrastructure
de données géographiques (IDS, IDG) et de plateforme logicielles
