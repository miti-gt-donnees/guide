...
Il convient de prendre en compte les besoins des utilisateurs du projet, quelquefois appelés "use case", et les besoins entre les différents systèmes qui seront sollicités qu'ils existent déjà ou qu'ils soient élaborés pour le projet.
...
À ce stade, il est important de prévoir une sauvegarde pour les données qui seront collectées, créées, construites dans le cadre du projet. 
...
- Les infrastructures informatiques ont elles suffisament de stockage disponible installations ?   
- Etc...  
...
À noter qu'il existe des modèles de [plans de gestion de données dits "de structure"](https://urfistinfo.hypotheses.org/3555) dont la période considérée s'étend au-delà de la durée d'un seul projet. Ce type de modèle s'applique aux plateformes et est donc sans doute de façon générale plus adapté aux besoins du personnel technique. 
...
Enfin, il peut être souhaitable d'associer un Identifiant Universel Unique (UUID) avec PostgreSQL. Nicolas Raidelet explique comment faire dans cette présentation.
